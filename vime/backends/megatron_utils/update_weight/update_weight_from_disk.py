...
class UpdateWeightFromDisk:
                ...
                ray.get(
                    [
                        engine.update_weights_from_disk.remote(
                            model_path=str(version_dir), weight_version=str(self.weight_version)
                        )
    ...
    def _write_full_checkpoint(self, version_dir: Path) -> None:
        ...
        chunk_iterator = iter(
            self._iterator.get_hf_weight_chunks(self.weights_getter(), progress_desc="Save full disk checkpoint")
        )
        chunk_index = 0
        while True:
            try:
