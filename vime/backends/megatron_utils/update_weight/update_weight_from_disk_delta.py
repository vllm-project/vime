...
class UpdateWeightFromDiskDelta:
    """
    Delta weight sync over a shared filesystem. All ranks participate in HF conversion;
    rank zero diffs the gathered tensors against a CPU snapshot and publishes a version.
    Each engine fans out pull_weights through collective_rpc to materialize the checkpoint
    on every rollout host, then reloads it through update_weights_from_disk.
    """
    ...
    def _iter_hf_tensors(self):
        """Every rank participates in HF conversion; rank zero publishes."""
        for chunk in self._iterator.get_hf_weight_chunks(self.weights_getter()):
            if dist.get_rank() == 0:
                yield from chunk
