...
# Per-layer buffers for stacked expert tensors. vllm's Gemma4 loader expects
...
def _get_config(args):
    ...
    if checkpoint not in _config_cache:
        ...
        if hasattr(hf_text, "per_layer_config"):
            ...
            local_config = hf_text
            global_config = hf_text
        _config_cache[checkpoint] = {
            "global_attn_layers": {i for i, t in enumerate(hf_text.layer_types) if t == "full_attention"},
            "local_head_dim": local_config.head_dim,
            "global_head_dim": global_config.head_dim,
            "num_attention_heads": local_config.num_attention_heads,
            "local_num_kv_heads": local_config.num_key_value_heads,
            "global_num_kv_heads": global_config.num_key_value_heads,
...
def convert_gemma4_to_hf(args, name, param):
    ...
    if name == "module.module.output_layer.weight":
        return []  # tied to embedding weight
