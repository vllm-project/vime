...
Text request body::

    {
        ...
        "model": <teacher model>,      # OPTIONAL; only sent when --opd-teacher-model
                                       # is set (single-model servers use their loaded
                                       # model; vLLM accepts a missing model).
        "sampling_params": {
            "max_tokens": 1,           # endpoint requires >=1; generated token ignored
            "temperature": 0,
...
Multimodal request: first POST the (text + image_url) messages to the teacher's
``/v1/chat/completions/render`` to obtain ``token_ids`` + ``features``, then attach
the student's canonical full token_ids (re-aligning the feature placeholders) and
score with the same ``sampling_params`` — mirroring the rollout's disaggregated MM
flow.
