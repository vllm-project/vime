...
def _parse_vllm_choice(choice: dict[str, Any]) -> tuple[list[int], list[float], dict[str, Any]]:
    ...
    """
    tokens = [int(t) for t in (choice.get("token_ids") or [])]
    ...
    return tokens, log_probs, {"finish_reason": finish}
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        cur_response_token_ids, cur_log_probs, meta_info = _parse_vllm_choice(output["choices"][0])
