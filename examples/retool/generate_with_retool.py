...
#
# vime/vLLM counterpart of slime's ``examples/retool/generate_with_retool.py``.
# The only structural change versus slime is the rollout engine: requests go to
# vLLM's ``/inference/v1/generate`` (token-only body, ``token_ids`` not
# ``input_ids``) and the response ``choices[0]`` is normalized into a
# sglang-shaped ``meta_info`` (``finish_reason.type`` + ``output_token_logprobs``)
# via the canonical helpers in :mod:`vime.rollout.vllm_rollout`, so the
# multi-turn tool-call control flow below is identical to slime's.
import re
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    # Track the finish reason of the most recent turn so the post-loop status
    # mapping mirrors slime (which read it back off the last response).
    finish_type = "stop"
    ...
    for turn in range(TOOL_CONFIGS["max_turns"]):
        ...
        # Use token IDs instead of text. vLLM's /inference/v1/generate is
        # token-only; sampling params are nested under their own key and built
        # by the canonical helper (logprobs enabled there).
        current_token_ids = _coerce_flat_int_token_ids(prompt_tokens_ids + response_token_ids)
        payload = {
            "model": args.hf_checkpoint,
            "token_ids": current_token_ids,
            "sampling_params": _build_inference_sampling_params(per_turn_sampling_params),
        }
        ...
        # Normalize the vLLM choice into a sglang-shaped meta_info: finish
        # reason wrapped as {"type": str} and per-token (logprob, token_id)
        # pairs under output_token_logprobs, exactly what the non-streaming
        # vllm_rollout.generate produces. The retool control flow below then
        # reads meta_info the same way slime did.
        choice = output["choices"][0]
        meta_info = _vllm_meta_from_generate_choice(args, choice, output.get("usage"))
        cur_token_ids, cur_logprobs = _inference_generate_tokens_and_logprobs(choice)
        if cur_token_ids:
            meta_info["output_token_logprobs"] = [
                [float(lp), int(tid)] for lp, tid in zip(cur_logprobs, cur_token_ids, strict=False)
            ]
        ...
        if not meta_info.get("output_token_logprobs"):
            # vLLM returned a choice but no per-token logprobs — we cannot
        ...
        cur_log_probs = [item[0] for item in meta_info["output_token_logprobs"]]
    ...
    # Set status (skip if a context-budget branch above already set it)
