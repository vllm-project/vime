...
- **Two teacher modes**:
  - **vllm**: Teacher runs on an external vLLM server, teacher log-probs are obtained during rollout via `/v1/completions` with `prompt_logprobs`.
...
|------|------------------|-------------|
| `vllm` | External vLLM server | Teacher has different architecture or larger than GPU memory |
...
1. **Why are there two OPD modes?**
   - `vllm` mode: The teacher runs on an independent vLLM server. This is useful when the teacher has a different architecture or is too large to load together with the policy model.
