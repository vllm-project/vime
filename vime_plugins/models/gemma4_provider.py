...
from vime_plugins.models.gemma4 import _load_hf_text_config
...
def _mark_expert_weights_for_direct_hf_loading(model):
    ...
    for name, parameter in model.named_parameters():
        if ".mlp.experts.linear_fc1.weight" in name:
            ...
            set_tensor_model_parallel_attributes(parameter, True, 1, 1)


def _promote_layer_scalars(model):
    for module in model.modules():
        layer_scalar = module._buffers.pop("layer_scalar", None)
        if layer_scalar is not None:
            module.register_parameter("layer_scalar", torch.nn.Parameter(layer_scalar, requires_grad=False))
...
def _install_hooks(model, args, config, pre_process, post_process):
    ...
    if hasattr(inner, "rotary_pos_emb"):
        ...
        global_head_dim = config.global_kv_channels
