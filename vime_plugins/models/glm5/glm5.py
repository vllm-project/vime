...
class _VLLMAbsorbWeightSTE(torch.autograd.Function):
    @staticmethod
...
def _get_vllm_indexer_head_weights(
    ...
    *,
    ...
    if hidden_states.dtype != torch.bfloat16 or weight.dtype != torch.bfloat16:
        raise TypeError(
            "VLLM indexer head-gate alignment requires BF16 input and weight, "
            f"got {hidden_states.dtype}/{weight.dtype}"
        )
    output = _VLLMIndexerHeadWeights.apply(hidden_states, weight)
...
class _VLLMRoPE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, value, cos_sin_cache, positions):
        ctx.save_for_backward(cos_sin_cache, positions)
        return _apply_vllm_rope_forward(value, cos_sin_cache, positions)
...
def _get_vllm_rope_cache(
    ...
    cache = _VLLM_ROPE_CACHE.get(key)
...
def _fake_quant_fp8_kv_cache(kv: torch.Tensor) -> torch.Tensor:
    ...
    if block_size != 128:
        raise ValueError("VLLM-aligned KV FP8 QAT requires block size 128")
...
class DSAMultiLatentAttention(Attention):
    ...
    def forward(
        ...
        *,
        ...
        if os.getenv("MEGATRON_USE_VLLM_SPARSE_MLA", "0") == "1":
            core_attn_out, _ = VLLMSparseMLA.apply(
                q,
                kv,
                topk_indices,
                self.softmax_scale,
    ...
    def get_absorb_query_key_value_tensors(
        ...
        *,
        ...
        def fuse_rope(q, cu_seqlens, gathered=False):
            if os.getenv("MEGATRON_USE_VLLM_ROPE", "0") == "1":
                ...
                if torch.is_grad_enabled() and q.requires_grad:
                    return _VLLMRoPE.apply(q, cache, positions)
                return _apply_vllm_rope_forward(q, cache, positions)
        ...
        head_weights = _get_vllm_indexer_head_weights(
            hidden_states,
