...
Extends the native Gemma3 implementation with Gemma4-specific features:
...
class Gemma4TransformerLayerSubmodules(TransformerLayerSubmodules):
    ...
    # keeps the `.mlp.experts.linear_fc...` naming that the weight-conversion path
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_moe_block:
            ...
            # attribute is kept on the MoE block so weight-conversion paths
            # don't change.
...
class SDPACoreAttention(nn.Module):
    ...
    Dispatch at call time (packed / thd shape):
      - CP > 1 (any layer) : all-gather K/V, apply causal + optional
        sliding-window mask computed from vime zig-zag global indices.
    ...
    def _zigzag_global_indices(local_len, cp_rank, cp_size, device):
        """Global positions of this rank's local Q tokens under vime's
    ...
    def _forward_cp_subseq_mask(self, query, key, value, packed_seq_params, sliding_window=None):
        ...
        Under vime's CP convention, ``packed_seq_params.cu_seqlens_q`` holds
...
def get_gemma4_layer_spec_te(config=None) -> ModuleSpec:
    ...
    ``.mlp.experts.linear_fc*.weight*`` matches the weight-conversion path),
...
def _load_hf_text_config(hf_checkpoint):
    ...
    Cached via lru_cache so the model provider and weight converters share the
    same parsed object.
    """
...
def _apply_core_config(config, hf_text):
    ...
    if hasattr(hf_text, "per_layer_config"):
        global_config = next(
            layer_config
            for layer_type, layer_config in zip(hf_text.layer_types, hf_text.per_layer_config, strict=True)
            if layer_type == "full_attention"
        )
        config.global_kv_channels = global_config.head_dim
        config.global_num_query_groups = global_config.num_key_value_heads
    else:
...
def get_rope_local_base_freq(hf_text) -> float:
    ...
    Single source of truth for both the model provider and model config path;
    otherwise the 10000.0 default would drift between
    call sites.
    """
