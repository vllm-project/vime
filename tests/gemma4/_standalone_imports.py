...
def load_gemma4_provider_module():
    ...
    gemma4_stub = types.ModuleType("vime_plugins.models.gemma4")
    gemma4_stub._load_hf_text_config = lambda path: None

    with _temporary_module("vime_plugins.models.gemma4", gemma4_stub):
        spec = importlib.util.spec_from_file_location(
            "_gemma4_provider_under_test",
            _repo_path("vime_plugins/models/gemma4_provider.py"),
        )
...
def load_gemma4_bridge_class():
    ...
    with _temporary_module("vime_plugins.models.gemma4", gemma4_stub):
        spec = importlib.util.spec_from_file_location(
            ...
            _repo_path("vime_plugins/mbridge/gemma4.py"),
        )
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
...
def load_gemma4_model_module():
    ...
    spec = importlib.util.spec_from_file_location(
        ...
        _repo_path("vime_plugins/models/gemma4.py"),
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
